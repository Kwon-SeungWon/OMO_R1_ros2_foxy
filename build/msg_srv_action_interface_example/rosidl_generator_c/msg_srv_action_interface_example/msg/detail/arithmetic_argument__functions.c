  ...
  msg_srv_action_interface_example__msg__ArithmeticArgument__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    msg_srv_action_interface_example__msg__ArithmeticArgument * data =
      (msg_srv_action_interface_example__msg__ArithmeticArgument *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!msg_srv_action_interface_example__msg__ArithmeticArgument__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          msg_srv_action_interface_example__msg__ArithmeticArgument__fini(&data[i]);
        }
        free(data);
        return false;
      }
